...
class GridChoiceMenu():
  def __init__(self, data: List, display_values: Optional[List]=None, title: Optional[str]=None, data_type: str = "text",
    ...
    self._option_data = self._build_data(data, display_values, self._data_type, self._row_width)
    self._data = data
    self._display_values = display_values
    self._item_size = self._determine_item_size(self._option_data, self._row_width)
    ...
    self._cached_data_surf: engine.Surface = self._build_data_surf()
  ...
  def _create_arrows(self):
    ...
    self.lscroll_arrow = ScrollArrow('left', (left + h_offset, top + arrow_height))
    self.rscroll_arrow = ScrollArrow('right', (left + width + h_offset, top + arrow_height))
    if self.num_rows() == 1:
  ...
  def move_cursor(self, idx):
    ...
    scroll_to = self._identify_minimum_scroll_to_loc(tuple(map(int, self._scroll)), tuple(map(float, as_coord)))
  ...
  def _is_option_visible(self, idx: int) -> bool:
    ...
    bottom_right_coord = tuple(map(ceil, tuple_add(self._scroll, self._grid_size)))
  ...
  def _get_pixel_size(self) -> Tuple[int, int]:
    ...
    if self._title:
      title_width, title_height = text_width('text', self._title), font_height('text')
  ...
  def _build_data(self, data: List[Any], display_values: Optional[List[Any]], data_type: str, row_width: int) -> List[IMenuOption]:
    ...
    for i in range(len(data)):
      ...
      new_option = ChoiceMenuOptionFactory.create_option(self._resolve_data_type(data_type), i, value, disp_value, row_width, self._text_align)
  ...
  def _build_data_surf(self, exclude_highlight=False) -> engine.Surface:
  ...
  def _get_data_surf(self) -> engine.Surface:
    ...
    option_x, option_y = tuple_add(self._get_pixel_coord_of_coord(cursor_coord), padding)
    ...
    choices = engine.subsurface(blank_surf, (offset_x, offset_y, gridw + px, gridh + py))
    return choices, padding

  def _get_cached_data_surf(self) -> Tuple[engine.Surface, Tuple[int, int]]:
  ...
  def _draw_grid(self, surf: engine.Surface, options_top_left: Tuple[int, int]):
    if self._is_option_oversize(): # redraw the entire grid each time
      data_surf, data_offset = self._get_data_surf()
    else:
      data_surf, data_offset = self._get_cached_data_surf()
  ...
  def _draw_other_ui(self, surf, options_top_left) -> engine.Surface:
    ...
    if self._should_draw_scrollbar:
      ...
      if total_height > self.num_rows():
        ...
        self.scroll_bar.draw(surf, (x, y), int(self._scroll[1]), self.num_rows(), total_height, self._item_size[1])
