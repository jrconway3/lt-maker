...
from app.engine import engine, info_menu, evaluate
...
class FreeRoamState(MapState):
    ...
    def take_input(self, event):
        ...
        if 'UP' in directions and self.road_unit.position[1] > 0:
            self.last_move = engine.get_time()
            if self.can_move('UP'):
                self.move(0, -0.2)
                self.roam_unit.sprite.change_state('moving')
                ...
                SOUNDTHREAD.play_sfx('Error')
        elif 'DOWN' in directions and self.road_unit.position[1] < game.tilemap.height - 1:
            self.last_move = engine.get_time()
        ...
        if event == 'SELECT':
            ...
            if other_unit:
                ...
                game.events.trigger('on_talk', self.roam_unit, other_unit)
            elif region:
                SOUNDTHREAD.play_sfx('Select 2')
                game.events.trigger(region.sub_nid, self.roam_unit, position=self.roam_unit.position, region=region)
            else:
                SOUNDTHREAD.play_sfx('Error')
    ...
    def update(self):
        super().update()
        if self.last_move and self.last_move - engine.get_time() > 166:
            self.last_move = 0
    ...
    def can_move(self, unit, direc: str) -> bool:
        if direc == 'LEFT':
    ...
    def rationalize(self):
        ...
        """
        self.roam_unit.position = (int(round(self.roam_unit.position[0])), int(round(self.roam_unit.position[1])))
    ...
    def can_talk(self):
        ...
        for unit in game.units:
            if unit.position and unit is not self.roam_unit and \
                ...
                units.append(unit)
        units = list(sorted(units, lambda unit: utils.calculate_distance(self.roam_unit.position, unit.position)))
        if units:
            return units[0]
        return None
    ...
    def can_visit(self):
        ...
        for region in game.level.regions:
            if region.region_type == 'event' and region.contains(self.roam_unit.position):
                try:
