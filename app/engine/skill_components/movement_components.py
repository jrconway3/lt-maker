    ...
    def witch_warp(self, unit: UnitObject) -> list:
        ...
        for val in self.value:
            ...
            if u and u.position:
                ...
                continue
            if partner_pos:
                positions += [pos for pos in game.target_system.get_adjacent_positions(partner_pos) if movement_funcs.check_weakly_traversable(unit, pos) and not game.board.get_unit(pos)]
        return positions
            ...
            if target.position:
                try:
                    if evaluate.evaluate(self.value, target, unit, target.position):
                        positions += game.target_system.get_adjacent_positions(target.position)
